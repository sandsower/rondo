defmodule Rondo.PathSafety do
  ...
  defp resolve_segments(root, resolved_segments, [], _depth), do: {:ok, join_path(root, resolved_segments)}

  defp resolve_segments(_root, _resolved_segments, _segments, depth) when depth > @max_symlink_depth do
    {:error, :symlink_loop}
  end

  defp resolve_segments(root, resolved_segments, [segment | rest], depth) do
    ...
    case File.lstat(candidate_path) do
      ...
      {:ok, _stat} ->
        resolve_segments(root, resolved_segments ++ [segment], rest, depth)
