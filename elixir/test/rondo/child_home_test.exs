defmodule Rondo.ChildHomeTest do
  ...
  test "prepares a private synthetic home and replaces inherited environment" do
    ...
    for path <- [envelope.home_path, envelope.environment["TMPDIR"], envelope.environment["GH_CONFIG_DIR"]] do
  ...
  test "rejects a pre-existing symlink instead of changing its target permissions" do
    root = Path.join(System.tmp_dir!(), "rondo-child-home-symlink-#{System.unique_integer([:positive])}")
    ...
    File.mkdir_p!(Path.dirname(envelope.home_path))
    File.ln_s!(target, envelope.home_path)

    assert {:error, {:unexpected_child_home_entry, path, :symlink}} = ChildHome.prepare(envelope)
    assert path == envelope.home_path
  end
