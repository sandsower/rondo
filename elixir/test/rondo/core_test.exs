defmodule Rondo.CoreTest do
  ...
  test "Rondo.start_link delegates to the orchestrator" do
    ...
    assert Process.whereis(Rondo.Orchestrator) == pid
  ...
  test "non-active issue state stops running agent without cleaning workspace" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "rondo-elixir-nonactive-reconcile-#{System.unique_integer([:positive])}"
      )
  ...
  test "terminal issue state stops running agent and cleans workspace" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "rondo-elixir-terminal-reconcile-#{System.unique_integer([:positive])}"
      )
  ...
  test "normal worker exit schedules active-state continuation retry" do
    ...
    assert_due_after_range(due_at_ms, before_down_ms, after_state_ms, 1_000)
  end
  ...
  defp assert_due_after_range(due_at_ms, before_down_ms, after_state_ms, delay_ms) do
    assert due_at_ms >= before_down_ms + delay_ms
    assert due_at_ms <= after_state_ms + delay_ms
  end
  ...
  test "prompt builder reports workflow load failures separately from template parse errors" do
    original_workflow_path = Workflow.workflow_file_path()
    workflow_store_pid = Process.whereis(Rondo.WorkflowStore)

    on_exit(fn ->
      Workflow.set_workflow_file_path(original_workflow_path)

      if is_pid(workflow_store_pid) and is_nil(Process.whereis(Rondo.WorkflowStore)) do
        Supervisor.restart_child(Rondo.Supervisor, Rondo.WorkflowStore)
      end
    end)

    assert :ok = Supervisor.terminate_child(Rondo.Supervisor, Rondo.WorkflowStore)
  ...
  test "agent runner keeps workspace after successful claude run" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "rondo-elixir-agent-runner-retain-workspace-#{System.unique_integer([:positive])}"
      )
  ...
  test "agent runner forwards timestamped claude updates to recipient" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "rondo-elixir-agent-runner-updates-#{System.unique_integer([:positive])}"
      )
  ...
  test "agent runner continues with a follow-up turn while the issue remains active" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "rondo-elixir-agent-runner-continuation-#{System.unique_integer([:positive])}"
      )
  ...
  test "agent runner stops continuing once agent.max_turns is reached" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "rondo-elixir-agent-runner-max-turns-#{System.unique_integer([:positive])}"
      )
