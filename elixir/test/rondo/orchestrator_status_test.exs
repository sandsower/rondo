defmodule Rondo.OrchestratorStatusTest do
  ...
  describe "escalation integration" do
    test "retry state preserves escalation metadata through schedule/pop round-trip" do
      workspace_root = tmp_dir("orchestrator-escalation-retry")

      write_workflow_file!(Workflow.workflow_file_path(),
        tracker_kind: "memory",
        workspace_root: workspace_root,
        escalation_enabled: true,
        escalation_tiers: ["light", "standard", "heavy"],
        escalation_max_total_attempts: 5
      )

      issue = %Issue{
        id: "issue-escalation-retry",
        identifier: "MT-ESC-RETRY",
        title: "Escalation retry test",
        description: "Verify escalation metadata survives retry state round-trip",
        state: "Todo",
        url: "https://example.org/issues/MT-ESC-RETRY"
      }

      Application.put_env(:rondo, :memory_tracker_issues, [issue])

      orchestrator_name = Module.concat(__MODULE__, :EscalationRetryOrchestrator)
      {:ok, pid} = Orchestrator.start_link(name: orchestrator_name)
      ...
      # Inject a retry entry with escalation metadata directly into state
      retry_token = make_ref()
      attempt_chain = [%{run_id: "run-1", tier: "light", reason: :initial, status: :failed, token_spend: %{total_tokens: 10}}]
      ...
      new_state = %{
        ...
        | retry_attempts: %{
            issue.id => %{
              ...
              identifier: "MT-ESC-RETRY",
              error: "agent exited: task_failure",
              failure_reason: nil,
              delay_type: :escalation,
              next_tier: "standard",
              evidence_prompt: "This is an escalated attempt at tier `standard`.",
      ...
      # Fire the retry and capture the metadata extracted by pop_retry_attempt_state
      send(pid, {:retry_issue, issue.id, retry_token})
      Process.sleep(100)

      # The retry entry should be gone (popped)
      state = :sys.get_state(pid)
      refute Map.has_key?(state.retry_attempts, issue.id)
    end

    test "escalation config is loaded correctly from workflow file" do
      workspace_root = tmp_dir("orchestrator-escalation-config")
      ...
      write_workflow_file!(Workflow.workflow_file_path(),
        ...
        escalation_tiers: ["light", "standard", "heavy"],
        escalation_max_total_attempts: 5,
        escalation_token_budget: 1_000_000,
        escalation_report_repair_attempts: 3
      )

      orchestrator_name = Module.concat(__MODULE__, :EscalationConfigOrchestrator)
      {:ok, pid} = Orchestrator.start_link(name: orchestrator_name)
      ...
      config = Rondo.Escalation.resolve_config(nil)
      assert config.enabled == true
      assert config.tiers == ["light", "standard", "heavy"]
      assert config.max_total_attempts == 5
      assert config.token_budget == 1_000_000
      assert config.report_repair_attempts == 3
    end

    test "escalation decision with task_failure manifest produces escalate with evidence prompt" do
      workspace_root = tmp_dir("orchestrator-escalation-decision")
      ...
      # Create a run manifest with failure data
      issue = %Issue{
        id: "issue-esc-decision",
        identifier: "MT-ESC-DECISION",
        title: "Escalation decision test",
        description: "Verify escalation decision from manifest",
        state: "Todo",
        url: "https://example.org/issues/MT-ESC-DECISION"
      }

      assert {:ok, ledger} =
               Rondo.RunLedger.create_run(issue,
                 workspace_root: workspace_root,
                 random_suffix: "decaf00d"
               )
      ...
      # Update the manifest to simulate a failed run with task_failure
      manifest = ledger.manifest_path |> File.read!() |> Jason.decode!()

      manifest =
        manifest
        |> Map.put("status", "failed")
        |> Map.put("failure_classification", "task_failure")
        |> Map.put("latest_gate", %{"status" => "fail", "failed" => [%{"name" => "test"}]})
        |> put_in(["agent", "model_routing"], %{"requested_tier" => "light", "resolved" => %{"model" => "claude-sonnet-4-20250514"}})
        |> put_in(["agent", "usage"], %{"input_tokens" => 1, "output_tokens" => 2, "total_tokens" => 3})
        |> put_in(["final_report"], %{"status" => "missing", "errors" => []})
        |> put_in(["timestamps", "finished_at"], DateTime.to_iso8601(DateTime.utc_now()))

      File.write!(ledger.manifest_path, Jason.encode!(manifest))

      # Load the manifest and evaluate escalation
      assert {:ok, loaded_manifest} = Rondo.RunLedger.load_manifest(ledger.run_dir)

      assert {:escalate, next_tier, chain, prompt} =
               Rondo.Escalation.after_attempt(loaded_manifest, [], nil)

      assert next_tier == "standard"
      assert length(chain) == 1
      assert List.first(chain).tier == "light"
